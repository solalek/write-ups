...
This is a write-up for the challenge `picoCTF - vault-door-8`.
...
Firstly, we will format the code:
``` Java
// These pesky special agents keep reverse engineering our source code and then
// breaking into our secret vaults. THIS will teach those sneaky sneaks a
// lesson.
...
3. The function encodes flag using the `scramble` function.

---

## Solution

To decode the flag, we will write a function to reverse the scrambling.
``` Java
public char[] unscramble(char[] passChars) {
...
```
And we will use it in the `checkPassword` function:
``` Java
    public boolean checkPassword(String password) {
        char[] scrambled = scramble(password);
        char[] expected = { 0xF4, 0xC0, 0x97, 0xF0, 0x77, 0x97, 0xC0, 0xE4, 0xF0, 0x77, 0xA4, 0xD0, 0xC5, 0x77, 0xF4, 0x86, 0xD0, 0xA5, 0x45, 0x96, 0x27, 0xB5, 0x77, 0xE0, 0x95, 0xF1, 0xE1, 0xE0, 0xA4, 0xC0, 0x94, 0xA4 };
        ...
        for (int i = 0; i < unscrambled.length; i++) {
            System.out.print(unscrambled[i]);
        }
        System.out.println();
        return Arrays.equals(scrambled, expected);
    } 
...
``` bash
java VaultDoor8                                                                                              403ms
Enter vault password: picoCTF{...}
s0m3_m0r3_b1t_sh1fTiNg_2e762b0ab
Access denied!
```

The flag: `picoCTF{s0m3_m0r3_b1t_sh1fTiNg_2e762b0ab}`
